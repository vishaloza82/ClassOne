//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace ClassOne.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class Package_Mapping_Products
    {
        public int PackageMappingProductId { get; set; }
        public int PackageId { get; set; }
        public int ProductId { get; set; }
        public int Quantity { get; set; }
        public System.DateTime CreatedDate { get; set; }
        public System.DateTime UpdatedDate { get; set; }
        public int DeleteStatus { get; set; }
        public Nullable<bool> ProductStatus { get; set; }
        public bool IsMandatoryFlag { get; set; }
        public int MaxPurchaseLimit { get; set; }
        public int MinPurchaseLimit { get; set; }
        public bool ChangeQuantityFlag { get; set; }
        public Nullable<bool> AvailableForDelivery { get; set; }
        public bool SaleClubbed { get; set; }
        public bool PurchaseFromMarketFlag { get; set; }
    
        public virtual Package Package { get; set; }
        public virtual Product Product { get; set; }
    }
}
